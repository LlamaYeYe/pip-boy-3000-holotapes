(function () {
  ...
  function playSound(type: string): void {
    if (Pip.playSound) {
      try {
        Pip.playSound(type as PipSoundName);
      } catch (e) {}
    }
  }

  function isP(char: string, player: string): boolean {
  ...
  function calculateValidMovesCache(
    or: number,
    oc: number,
    piece: string,
  ): void {
  ...
  function isKingInCheck(playerColor: string): boolean {
  ...
  function hasLegalMoves(playerColor: string): boolean {
    for (let r = 0; r < 8; r++) {
      for (let c = 0; c < 8; c++) {
        let p = board[r * 8 + c];
  ...
  function countPieces(playerColor: string): number {
    let count = 0;
    for (let i = 0; i < 64; i++) {
  ...
  function drawBtn(label: string, yPos: number, isSelected: boolean): void {
    if (isSelected) {
  ...
  function resolvePostMoveEffects(): void {
  ...
  function handleSelectionClick(): void {
    if (phase === 'MAIN_MENU') {
      playSound('SELECT');
      if (menuSel === 0) {
        gameMode = 'TWO_PLAYER';
  ...
  function handleMenuScroll(dir: number, max: number): void {
    menuSel = dir > 0 ? (menuSel + 1) % max : (menuSel - 1 + max) % max;
    draw();
  }

  function onWheel(knob: number, dir: number): void {
    if (phase === 'AI_THINKING') return;
    if (dir === 0) {
  ...
  for (let i = 0; i < 64; i++) validTargets[i] = false;
  Pip.onExclusive('knob1', function (d: number) {
    onWheel(1, d);
  });
  Pip.onExclusive('knob2', function (d: number) {
    onWheel(2, d);
  });
  draw();
  ...
  instance = {
    ...
    remove: function () {
      if (removed) return;
      removed = true;
      (Pip.removeListener as (event: string) => void)('knob1');
      (Pip.removeListener as (event: string) => void)('knob2');
      if (Pip.audioStop) Pip.audioStop();
      h.clear();
    },
  };
