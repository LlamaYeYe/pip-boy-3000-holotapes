...
(function () {
  ...
  let tableau: SolitaireCard[][],
    ...
    waste: SolitaireCard[],
    foundations!: number[],
    held: SolitaireHeld | null,
    cursorPile!: number,
    grabDepth!: number;
  let gameState!: number,
    redrawInterval!: number,
    winInterval: number | undefined,
    winQueue!: number[],
    winIdx!: number,
    bouncer: SolitaireBouncer | null;
  let deckIdx = 0;
  let loadedDeckImg: GraphicsImageObject | null = null;
  ...
  function rankIdx(c: SolitaireCard): number {
    return c % 13;
  }
  function suitIdx(c: SolitaireCard): number {
    return (c / 13) | 0;
  }
  function rank1(c: SolitaireCard): number {
    return rankIdx(c) + 1;
  }
  function isRed(c: SolitaireCard): boolean {
    let s = suitIdx(c);
    return s === 1 || s === 2;
  }
  ...
  function loadSelectedDeck(): void {
    ...
    try {
      ...
      const image: GraphicsImageObject = {
        bpp: data.bpp,
  ...
  function dealGame(): void {
    ...
    for (let i = 0; i < 7; i++) {
      for (let j = 0; j <= i; j++) {
        let c = d.pop();
        if (c !== undefined) tableau[i].push(c);
      }
      tabDown[i] = i;
    }
    stock = d;
  ...
  function pickUp(p: number): void {
    if (p === 1) {
      ...
      let i = p - 6,
        up = tableau[i].length - tabDown[i];
      if (up > 0) held = { from: p, count: Math.min(grabDepth, up) };
    }
  }

  function sourceCards(from: number, count: number): SolitaireCard[] {
  ...
  function placeCards(p: number, cards: SolitaireCard[]): void {
    if (p >= 2 && p < 6) {
      let suit = FOUND_SLOT_SUIT[p - 2];
  ...
  function tryPlace(p: number): void {
    if (!held) return;
    ...
    if (p >= 2 && p < 6) {
      ...
      if (
        held.count === 1 &&
        suitIdx(cards[0]) === suit &&
        rank1(cards[0]) === foundations[suit] + 1
      )
        ok = true;
    } else if (p >= 6) {
      let i = p - 6,
        n = tableau[i].length;
      if (n === 0) {
        if (rank1(cards[0]) === 13) ok = true;
      } else {
        let top = tableau[i][n - 1];
        if (
          rank1(top) === rank1(cards[0]) + 1 &&
          isRed(top) !== isRed(cards[0])
        )
          ok = true;
      }
    }
  ...
  function moveHoriz(dir: number): void {
    cursorPile =
      cursorPile < 6
        ? (cursorPile + dir + 6) % 6
        : 6 + ((cursorPile - 6 + dir + 7) % 7);
    grabDepth = 1;
  }

  function moveVert(): void {
    cursorPile =
      cursorPile < 6 ? 6 + TOP_TO_TAB[cursorPile] : TAB_TO_TOP[cursorPile - 6];
    grabDepth = 1;
  }

  function adjustDepth(dir: number): void {
    let i = cursorPile - 6,
      up = tableau[i].length - tabDown[i];
    if (up < 1) up = 1;
  ...
  function verticalMove(dir: number): void {
    if (cursorPile >= 6 && held === null) {
      let i = cursorPile - 6,
        up = tableau[i].length - tabDown[i];
      if (up < 1) up = 1;
      if (dir > 0 && grabDepth < up) {
        adjustDepth(dir);
        return;
      }
    }
  ...
  function slotForSuit(s: number): number {
    for (let j = 0; j < 4; j++) if (FOUND_SLOT_SUIT[j] === s) return j;
    return 0;
  }

  function enterWin(): void {
  ...
  function winTick(): void {
    'ram';
    if (!bouncer) {
      if (winIdx >= winQueue.length) {
        clearInterval(winInterval);
        winInterval = undefined;
        return;
      }
      let card = winQueue[winIdx++],
        slot = slotForSuit(suitIdx(card));
      bouncer = {
        card: card,
    ...
    if (bouncer.y + C.CARD_H / 2 > 320) {
      bouncer.y = 320 - C.CARD_H / 2;
      bouncer.vy = -bouncer.vy * 0.7;
      bouncer.b++;
    }
    if (bouncer.x - C.CARD_W / 2 < 0) {
      bouncer.x = C.CARD_W / 2;
      bouncer.vx = -bouncer.vx * 0.8;
    }
    if (bouncer.x + C.CARD_W / 2 > 480) {
      bouncer.x = 480 - C.CARD_W / 2;
      bouncer.vx = -bouncer.vx * 0.8;
    }
    drawCardFace(
      bouncer.card,
      bouncer.x - C.CARD_W / 2,
      bouncer.y - C.CARD_H / 2,
      true,
    );
    drawWinBox();
    h.flip();
  ...
  function drawHeart(
    ...
    if (outline) {
      ...
      h.drawCircle(cx + r * 0.5, cy - r * 0.3, r * 0.5);
      h.drawPoly(
        [cx - r, cy - r * 0.1, cx + r, cy - r * 0.1, cx, cy + r],
        true,
      );
    } else {
  ...
  function drawDiamond(
    ...
    if (outline) {
      h.drawPoly(
        [cx, cy - r, cx + r * 0.65, cy, cx, cy + r, cx - r * 0.65, cy],
        true,
      );
    } else {
      h.fillPoly([
  ...
  function drawCardBack(x: number, y: number, exposedH?: number): void {
  ...
  function drawEmptySlot(x: number, y: number): void {
  ...
  function layoutColumn(i: number): number[] {
    let n = tableau[i].length,
      ys = [C.TAB_Y];
    let avail = 320 - 16 - C.TAB_Y - C.CARD_H;
    let total = 0;
    for (let k = 1; k < n; k++)
      total += k - 1 < tabDown[i] ? C.FD_OFF : C.FU_OFF;
  ...
  function topBox(p: number, depth: number): number[] {
    if (p === 0)
      return [C.STOCK_X, C.ROW1_Y, C.STOCK_X + C.CARD_W, C.ROW1_Y + C.CARD_H];
    if (p === 1)
      return [C.WASTE_X, C.ROW1_Y, C.WASTE_X + C.CARD_W, C.ROW1_Y + C.CARD_H];
    if (p < 6) {
      let x = C.FOUND_X + (p - 2) * C.FOUND_SPACING;
      return [x, C.ROW1_Y, x + C.CARD_W, C.ROW1_Y + C.CARD_H];
    }
    let i = p - 6,
      x = C.TAB_X + i * C.TAB_SPACING,
      n = tableau[i].length;
  ...
  function hintText(): string {
    ...
    if (held)
      return held.from === cursorPile ? 'Press: Cancel' : 'Press: Place Here';
    if (cursorPile === 0)
      return stock.length
        ? 'Press: Draw Card'
        : waste.length
  ...
  function drawTitleScreen(): void {
    ...
    h.setColor(3)
      .setFontMonofonto36()
      .setFontAlign(0, 0)
      .drawString('SOLITAIRE', 240, 84);
    h.setColor(2)
      .setFontMonofonto18()
      .setFontAlign(0, 0)
      .drawString('Select a deck, then', 240, 150);
    h.setColor(2)
      .setFontMonofonto18()
      .setFontAlign(0, 0)
      .drawString('press the left wheel to play!', 240, 173);
  ...
  function drawWaste(): void {
    if (waste.length)
      drawCardFace(waste[waste.length - 1], C.WASTE_X, C.ROW1_Y, true);
    else drawEmptySlot(C.WASTE_X, C.ROW1_Y);
  }

  function drawFoundationSlot(j: number): void {
    let x = C.FOUND_X + j * C.FOUND_SPACING,
      suit = FOUND_SLOT_SUIT[j];
  ...
  function drawTableauColumn(i: number): void {
    let col = tableau[i],
      n = col.length,
      x = C.TAB_X + i * C.TAB_SPACING;
    if (n === 0) {
      drawEmptySlot(x, C.TAB_Y);
      return;
    }
  ...
  function drawPlayScreen(): void {
    h.setColor(2)
      .setFontMonofonto16()
      .setFontAlign(-1, 0)
      .drawString(
        'DECK: ' + stock.length,
        C.WASTE_X + C.CARD_W + 6,
        C.ROW1_Y + C.CARD_H / 2,
      );
    drawStock();
    drawWaste();
  ...
  function drawWinBox(): void {
    ...
    h.setFontMonofonto36()
      .setFontAlign(0, 0)
      .drawString('YOU WIN!!!', 240, 145);
    h.setColor(2)
      .setFontMonofonto18()
      .drawString('Press left wheel to play again!', 240, 190);
  }
