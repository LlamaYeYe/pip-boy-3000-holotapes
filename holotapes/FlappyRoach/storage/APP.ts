...
(function () {
  ...
  function drawSplat(x: number, y: number, s: number, t: number): void {
  ...
  // * * * * * * * * * * * * * * * Save High Score * * * * * * * * * * * * * * * * * *
  function saveHighScoreIfNeeded(s: number): void {
    if (s > highScore) {
      highScore = s;
  ...
  function flushScreen(): void {
    (C.clearRect as () => Graphics)();
  }

  function newPipe(): void {
  ...
  function drawCentered(text: string, y: number): void {
      ...
      function (e) {
        ...
        if (e.state) {
          ...
          let step = Math.max(1, Math.min(5, enc1fast >> 1)),
            dir = Number(e.state) ^ e.data ? step : -step;
