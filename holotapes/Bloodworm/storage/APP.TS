...
(function () {
  ...
  let players: BloodwormPlayer[] = [];
  let apple = { x: 0, y: 0 };
  let grass: Array<{ x: number; y: number }> = [];
  ...
  function clearTile(c: number, r: number): void {
    h.setColor(C_BLACK);
  ...
  function drawApple(c: number, r: number): void {
    const x = tileX(c);
    const y = tileY(r);
    h.setColor(C_BRIGHT);
  ...
  function drawDecoSnake(x: number, y: number): void {
    h.setColor(C_BRIGHT);
    for (let i = 0; i < 6; i++) {
      const sx = x + i * TILE;
  ...
  function drawGameOverScreen(reason?: string): void {
    h.clear(1);
    const cx = W / 2;
    const cy = H / 2;
  ...
  function drawGrass(c: number, r: number): void {
    const x = tileX(c);
    const y = tileY(r);
    h.setColor(C_DIM);
  ...
  function isOccupiedByAny(c: number, r: number, ignoreTail: boolean): boolean {
    for (let p = 0; p < players.length; p++) {
      const segs = players[p].segments;
  ...
  function mainLoop(): void {
    ...
    const newHeads: Array<BloodwormPoint | null> = [];
    for (let i = 0; i < players.length; i++) {
      const p = players[i];
      if (!p.alive) {
        newHeads.push(null);
  ...
  function spawnApple(): void {
    let nx!: number, ny!: number, ok!: boolean;
    let tries = 0;
    do {
  ...
  function tileX(c: number): number {
    return PLAY_X + c * TILE;
  }

  function tileY(r: number): number {
    return PLAY_Y + r * TILE;
  }

  function toggleModeFromMenu(dir: number): void {
    if (!dir) return;
    const now = Date.now();
  ...
  function turnPlayer(playerIdx: number, knobDir: number): void {
