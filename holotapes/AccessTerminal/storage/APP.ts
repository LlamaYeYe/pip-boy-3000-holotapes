...
(function () {
  ...
  let clickWatch: number | null = null;
  let consumedSpans: Record<string, boolean> = {};
  let cursorCol = 0;
  let cursorRow = 0;
  ...
  let lockoutInterval: number | null = null;
  let lockoutUntil = 0;
  let logEntries: string[] = [];
  let password = '';
  let playWatch: number | null = null;
  let removed = false;
  let rows: Array<{ line: string; candidate: number }> = [];
  let soundExists: Record<string, boolean> = {};
  let spans: AccessTerminalSpan[] = [];
  ...
  function canPlaceRange(line: string, start: number, len: number): boolean {
  ...
  function clearLockoutTimer(): void {
    if (lockoutInterval) {
  ...
  function drawAttemptCounter(): void {
  ...
  function drawEndScreen(title: string, subtitle: string): void {
  ...
  function drawGridColumn(left: boolean): void {
  ...
  function drawLockoutScreen(): void {
  ...
  function getCandidateInRow(rowIdx: number): {
    value: string;
    row: number;
    start: number;
    removed: boolean;
  ...
  function getMatchScore(guess: string, answer: string): number {
    let score = 0;
  ...
  function getSide(row: number): { left: boolean } {
    return { left: row < ROWS_PER_COLUMN };
  }

  function getGridRowY(rowInSide: number): number {
  ...
  function getSpanAtCursor(): AccessTerminalSpan | null {
  ...
  function handleBonus(span: AccessTerminalSpan): void {
    span.consumed = true;
  ...
  function handleClick(source: string): void {
    if (removed) return;
  ...
  function insertBonusInLine(rowIndex: number): boolean {
  ...
  function playSound(key: string): void {
    const path = (SOUND_PATHS as Record<string, string>)[key];
    if (!path) return;
  ...
  function playSoundLater(key: string, delay: number): void {
    setTimeout(function () {
  ...
  function removeSpansForRow(rowIndex: number): void {
    for (let i = spans.length - 1; i >= 0; i--) {
      if (spans[i].row === rowIndex) spans.splice(i, 1);
    }
  }

  function scanSpans(): void {
    const next: AccessTerminalSpan[] = [];
    const pairs = { '(': ')', '[': ']', '{': '}', '<': '>' };

    for (let r = 0; r < rows.length; r++) {
      const line = rows[r].line;
      for (let c = 0; c < line.length; c++) {
        const closer = (pairs as Record<string, string>)[line[c]];
        if (!closer) continue;
  ...
  function spanContainsActiveWord(
    rowIndex: number,
    start: number,
    end: number,
  ): boolean {
