...
(function () {
  ...
  const subtitles = eval(
    require('fs').readFileSync('HOLO/BOOK_IT/SUBS.JS'),
  ) as string[];
  let curSubtitle = subtitles[0];

  function pathJoin(a: string | null, b: string): string {
    if (!a) return b;
    if (!b) return a;
    return a + '/' + b;
  }
  ...
  function loadBrowser(): void {
    ...
    if (listIdx >= listItems.length)
      listIdx = Math.max(0, listItems.length - 1);
  ...
  function loadSave(): void {
    ...
    try {
      ...
      if (d) {
        let j = JSON.parse(E.toString(d) as string);
  ...
  function fetchPage(pIdx: number): void {
    if (pIdx < 0) pIdx = 0;
    ...
    let text = dump ? (E.toString(dump) as string) : '';
    f.close();
  ...
  function drawBrowser(): void {
    ...
    h.setColor(3)
      .setFontMonofonto36()
      .setFontAlign(0, 0)
      .drawString('BOOK-IT', 240, 28);
    h.setColor(2)
      .setFontMonofonto16()
      .setFontAlign(0, 0)
      .drawString(curSubtitle, 242, 65);
  ...
  function browserMove(dir: number): void {
    'ram';
    if (dir) {
      listIdx += dir;
      ...
      if (listItems.length > 0) {
        ...
        if (item.type === 'back') {
          ...
          currentFolder = item.name as string;
          loadBrowser();
          listIdx = 0;
          scrollOff = 0;
          draw();
          ...
          curFile = currentFolder
            ? pathJoin(currentFolder, item.name as string)
            : (item.name as string);
          loadSave();
          fetchPage(curPage);
          state = 1;
