...
(function () {
  ...
  let scores: Record<string, number> = {},
    qi = -1,
    sel = 0,
    wrapCache: GoatWrapCache | undefined;
  ...
  function wrap(i: number): GoatWrapCache {
    if (wrapCache && wrapCache.i === i) return wrapCache;
    const q = h.setFontMonofonto16().wrapString(Q[i][0] as string, 410);
    const a = (Q[i][1] as string[]).map((t: string) =>
      h.setFontMonofonto14().wrapString(t, 390),
    );
  ...
  function drawResult(): void {
    ...
    const job = (R as Record<string, string[]>)[best];
    h.clear(0);
    drawBorder();
  ...
  function onKnob1(dir: KnobDirection): void {
    ...
    if (dir) {
      ...
      Pip.playSound('TAB');
      const skill = (Q[qi][2] as string[])[sel];
