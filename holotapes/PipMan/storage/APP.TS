...
(function () {
  ...
  var dirX: number[] = [],
    dirY: number[] = [];
  var o0!: number, o1!: number, o2!: number, o3!: number, on!: number;
  var opx: number,
    opy: number,
    ogx = [0, 0, 0, 0],
    ogy = [0, 0, 0, 0];

  function tile(tx: number, ty: number): number {
  ...
  function markDirty(x: number, y: number): void {
    dirX.push(x);
    dirY.push(y);
  }

  function drawCell(c: number, r: number): void {
    var t = map[r * COLS + c];
    var x = OX + c * TW,
      y = OY + r * TH;
  ...
  function drawFullMaze(): void {
    for (var r = 0; r < ROWS; r++)
      for (var c = 0; c < COLS; c++) drawCell(c, r);
  }

  function drawSprite(c: number, r: number): void {
    var x = OX + c * TW,
      y = OY + r * TH;
    var cx = x + (TW >> 1),
      cy = y + (TH >> 1);
  ...
  function drawBox(l1: string, l2: string): void {
    var bw = 130,
      bh = l2 ? 38 : 20;
    var bx = (W - bw) >> 1,
      by = (H - bh) >> 1;
  ...
  function onK1(d: KnobDirection): void {
    if (!d) return;
    var n = Date.now();
    if (n - lk1 < 20) return;
    lk1 = n;
  ...
  function onK2(d: KnobDirection): void {
    if (!d) return;
    var n = Date.now();
    if (n - lk2 < 20) return;
    lk2 = n;
  ...
  function onBtn(e: WatchEvent): void {
    if (!e.state) return;
