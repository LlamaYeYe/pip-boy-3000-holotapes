...
(function () {
  ...
  let fadeOn: number | boolean = 0;
  let menuIndexSelected = 0;
  let fadeInterval: number | undefined = undefined;
  let roach1Image: RoachStompImage | null | undefined = undefined;
  let roach2Image: RoachStompImage | null | undefined = undefined;
  let boot1Image: RoachStompImage | null | undefined = undefined;
  let splat1Image: RoachStompImage | null | undefined = undefined;
  let heartFullImage: RoachStompImage | null | undefined = undefined;
  let heartEmptyImage: RoachStompImage | null | undefined = undefined;
  let score = 0;
  let isNewHighScore = false;
  let health = 0;
  let difficulty: RoachStompDifficulty | undefined = undefined;
  let assetsLoaded = false;
  let playerLaneIndexSelected = 0;
  let playerLaneMaxIndex = 0;
  let gameBoard!: Array<Array<{ x: number; y: number }>>;
  let playerBoard!: Array<{ x: number; y: number }>;
  let lanes!: RoachStompLane[];
  let dirtyLanes!: number[];
  let laneSpawnTimeouts!: Array<number | undefined>;
  let marchTimeout: number | undefined = undefined;
  let frameInterval: number | undefined = undefined;
  let gameOverInputDelay: number | undefined = undefined;
  let sfxWhooshRawInfo: RoachStompSfxInfo = {};
  let sfxSplatRawInfo: RoachStompSfxInfo = {};
  let sfxWhoosh: Uint8Array<ArrayBufferLike> | undefined = undefined;
  let sfxSplat: Uint8Array<ArrayBufferLike> | undefined = undefined;
  let clickWatch: number | undefined = undefined;
  let sfxHissRawInfo: RoachStompSfxInfo[] = [{}, {}, {}];
  let sfxHiss: Array<Uint8Array | undefined> = [
    undefined,
    undefined,
    undefined,
  ];
  let hissTimeout: number | undefined = undefined;
  ...
  function loadImage(path: string): RoachStompImage | null {
    try {
  ...
  function gameStartFade(): void {
    fadeOn = !fadeOn;
    drawTitleStartGame();
  }

  function loadHighScores(): RoachStompHighScore[] {
    try {
      return JSON.parse(
  ...
  function updateHealth(change: number): void {
    ...
    for (let i = 0; i < 3; i++) {
      if (i < 3 - health) {
        h.drawImage(heartEmptyImage as GraphicsImage, HEART_LOCS_X[i], 10);
      } else {
        h.drawImage(heartFullImage as GraphicsImage, HEART_LOCS_X[i], 10);
      }
    }
  ...
  function scoreTiersCrossed(): number {
  ...
  function currentMarchIntervalMs(): number {
    return Math.max(
  ...
  function currentStompWindowMs(): number {
    return Math.max(
  ...
  function playRandomHiss(): void {
    const i = Math.randInt(3);
    if (sfxHiss[i]) Pip.audioStartVar(sfxHiss[i]!, sfxHissRawInfo[i]);
  }

  function spawnGapMs(laneIndex: number): number {
  ...
  function countActiveRoaches(): number {
    let count = 0;
    for (let i = 0; i < lanes.length; i++) {
      if (lanes[i]) count++;
    }
  ...
  function scheduleMarchTick(): void {
  ...
  function checkStompTimeouts(): void {
    'ram';
    for (let i = 0; i < lanes.length; i++) {
      const roach = lanes[i];
      if (roach && roach.row === 2 && getTime() > (roach.stompDeadline || 0)) {
        lanes[i] = null;
        dirtyLanes[i] = 1;
        updateHealth(-1);
        if (health <= 0) {
  ...
  function onGameInterval(): void {
    ...
    for (let i = 0; i < lanes.length; i++) {
      if (lanes[i] && (lanes[i] as RoachStompRoach).row === 2)
        dirtyLanes[i] = 1;
    }
  ...
  function setLanes(count: number): void {
    lanes = [];
    dirtyLanes = [];
    laneSpawnTimeouts = undefined as never;
    for (let i = 0; i < count; i++) lanes.push(null);
    for (let i = 0; i < count; i++) dirtyLanes.push(0);
    for (let i = 0; i < count; i++) laneSpawnTimeouts.push(undefined);
  }
  ...
  function drawRoach(laneIndex: number): void {
    ...
    if (roach) {
      ...
      if (roach.row < 2) {
        h.drawImage(roach1Image as GraphicsImage, cell.x, cell.y);
      } else {
        h.drawImage(roach2Image as GraphicsImage, cell.x, cell.y);
        const remaining = E.clip(
          ((roach.stompDeadline || 0) - getTime()) /
            (currentStompWindowMs() / 1000),
          0,
          1,
        );
  ...
  function drawPlayer(newIndex: number, prevIndex: number): void {
    h.clearRect(
    ...
    h.drawImage(
      boot1Image as GraphicsImage,
      playerBoard[newIndex].x,
      playerBoard[newIndex].y,
    );
  }
  ...
  function drawMenuMain(): void {
    ...
    MENU_MAIN_OPTIONS.forEach((option: string | number, index) => {
  ...
  function drawTitleStartGame(): void {
  ...
  function onClick_InGame(event: WatchEvent): void {
    ...
    if (roach && roach.row === 2) {
      ...
      if (sfxSplat) Pip.audioStartVar(sfxSplat, sfxSplatRawInfo);
      h.drawImage(
        splat1Image as GraphicsImage,
        gameBoard[2][playerLaneIndexSelected].x,
        gameBoard[2][playerLaneIndexSelected].y,
      );
      updateScore(10);
    } else {
      if (sfxWhoosh) Pip.audioStartVar(sfxWhoosh, sfxWhooshRawInfo);
      h.drawImage(
        boot1Image as GraphicsImage,
        gameBoard[2][playerLaneIndexSelected].x,
        gameBoard[2][playerLaneIndexSelected].y,
  ...
  function onKnob2_InGame(dir: KnobDirection): void {
  ...
  function onKnob1_HighScores(dir: KnobDirection): void {
    if (dir === 0) {
  ...
  function onKnob1_MenuMain(dir: KnobDirection): void {
    if (dir === 0) {
  ...
  function onKnob1_TitleScreen(dir: KnobDirection): void {
    if (dir === 0) {
  ...
  function onKnob1_GameOver(dir: KnobDirection): void {
    if (dir === 0) {
