...
(function (app: RcetApp) {
  ...
  function drawBinImage(path: string): void {
  ...
  function drawEmptyState(): void {
    ...
    const lines = h.wrapString(
      'Add .IMG, .JSON, .JS (1/2 bpp), or full-screen .BIN images to the ' +
        'IMAGES folder on the SD card. Keep image files under 30 KB or load ' +
        'will fail. Convert images at https://pip-boy.com/tools',
      LIST_W - 8,
    );
  ...
  function drawError(name: string | number): void {
    h.clear(0);
    h.setColor(C_BRIGHT)
      .setFontMonofonto23()
      .setFontAlign(0, 0)
  ...
  function drawImageString(image: string): void {
  ...
  function drawJsonImage(path: string): void {
  ...
  function drawTitleBar(): void {
    h.setColor(C_BRIGHT)
      .setFontMonofonto16()
      .setFontAlign(-1, -1)
      .drawString(app.names[1], LIST_X + 6, LIST_TITLE_Y);

    const APP_VERSION = app.version;
    if (APP_VERSION) {
      const versionX = LIST_X + 6 + h.stringWidth(app.names[1]) + 4;
      h.setColor(C_BRIGHT)
        .setFont('6x8')
        .setFontAlign(-1, -1)
  ...
  function ellipsize(text: string, maxPx: number): string | number {
    'ram';
  ...
  function label(item: RcetListItem): string {
    if (item.type === 'image') return item.name as string;
  ...
  function openImage(name: string): void {
    sound('TAB');
    viewing = true;
    ...
    try {
      if (lower.slice(-4) === '.bin') {
        ...
        drawJsonImage(path);
      } else if (lower.slice(-4) === '.img') {
        drawImageString(fs.readFileSync(path));
      } else {
        drawImageString(eval(fs.readFileSync(path)) as string);
      }
    } catch (e) {
