...
(function () {
  ...
  let pending: CheckersPendingScene | null = null; // Next scene queued
  let scene: CheckersScene | null = null; // Current scene

  const app: CheckersApp = {
  ...
  function loadScene(): void {
    ...
    try {
      scene = (
        eval(fs.readFileSync('HOLO/CHECKERS/' + p.file)) as CheckersSceneFactory
      )(app, p.params);
    } catch (e) {
      drawError(p.file);
    }
