(function () {
  ...
  let grid: Array<Array<{ letter: string; state: number }>> = [];
  let row = 0;
  let col = 0;
  let answer = '';
  let phase = 'title';
  ...
  let keyStates: Record<string, number> = {};
  let resultDetail = '';

  let cursorRow = 0;
  let cursorCol = 0;

  let watchEnter: number | null = null;
  let watchBack: number | null = null;
  let soundCache: Record<string, boolean> = {};
  let stopped = false;

  function newGame(): void {
    grid = [];
    for (let r = 0; r < MAX_GUESSES; r++) {
      const r2: Array<{ letter: string; state: number }> = [];
  ...
  function playSound(name: string): void {
    const path = (SFX as Record<string, string>)[name];
    if (!path) return;
  ...
  function evaluateGuess(guess: string): number[] {
  ...
  function updateKeyStates(guess: string, result: number[]): void {
    for (let i = 0; i < WORD_LEN; i++) {
  ...
  function pressKey(label: string | null): void {
  ...
  function redrawCursor(prevRow: number, prevCol: number): void {
  ...
  function colorForState(state: number): 0 | 2 | 1 | 3 {
  ...
  function correctOutline(x: number, y: number, w: number, h2: number): void {
    h.setColor(0);
  ...
  function drawTile(
    x: number,
    y: number,
    cell: { letter: string; state: number },
  ): void {
    if (cell.state === EMPTY) {
  ...
  function drawMessageBox(title: string, detail: string, footer: string): void {
  ...
  function onPress(e: WatchEvent): void {
