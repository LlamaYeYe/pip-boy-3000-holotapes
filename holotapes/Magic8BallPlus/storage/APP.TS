...
(function () {
  ...
  let accelTimer: number | undefined;
  let idx = 0;
  let cache: string[][] = [];
  let shakeCount = 0;
  ...
  function drawTitleBg(): void {
    ...
    let file = E.openFile('HOLO/MAGIC_8_BALL_PLUS/TITLE.BIN', 'r');
  ...
  function drawTitle(): void {
    ...
    h.setColor(titleSelection === 0 ? 0 : 3)
      .setFontMonofonto23()
      .setFontAlign(0, 0);
    ...
    h.setColor(titleSelection === 1 ? 0 : 3)
      .setFontMonofonto23()
      .setFontAlign(0, 0);
  ...
  function getLines(i: number): string[] {
    if (cache[i]) return cache[i];

    const text =
      mode === 2 && i >= C.ANSWERS.length
  ...
  function drawBall(jx: number, jy: number, i: number): void {
    'ram';
  ...
  function pollAccelerometer(): void {
    'ram';
