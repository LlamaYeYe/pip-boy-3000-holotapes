...
(function () {
  ...
  let mode = 0,
    passIdx = 0;
  let latest: MesmetronSpiralGen | null = null; // most recently spawned generation - checked each frame
  // to see if it's crossed SPAWN_TICK yet
  let gens: MesmetronSpiralGen[] = []; // active generations - each an independent in-progress spiral
  let brightnessStep = 20,
    lastKnob = 0;

  function bisectAngle(n: number): number {
    let rev = 0,
      v = n;
  ...
  let setColor: (c: number) => Graphics;
  let drawLine: (x1: number, y1: number, x2: number, y2: number) => Graphics;

  function stroke(
    gen: MesmetronSpiralGen,
    slot: number,
    x: number,
    y: number,
  ): void {
  ...
  function drawGen(gen: MesmetronSpiralGen): boolean {
  ...
  function setup(variant: number): void {
    mode = variant;
    passIdx = 0;
    gens = [];
    latest = null;
