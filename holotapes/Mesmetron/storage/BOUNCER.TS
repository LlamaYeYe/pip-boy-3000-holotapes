...
(function () {
  ...
  let cx = 0,
    ...
    vx = 0,
    vy = 0;
  let sprite: GraphicsImageObject | null = null,
    spriteW = 0,
    spriteH = 0;
  let clearHalfW = 0,
    clearHalfH = 0;
  ...
  function loadSprite(filename: string): GraphicsImageObject {
  ...
  function setup(v: number): void {
    ...
    let angleDeg!: number, mod!: number;
    do {
  ...
  return {
    ...
    draw: function (h: Graphics) {
      ...
      if (variant === 0) {
        ...
        if (sprite) h.setColor(3).drawImage(sprite, cx | 0, cy | 0, rotOpts);
      } else {
        if (sprite)
