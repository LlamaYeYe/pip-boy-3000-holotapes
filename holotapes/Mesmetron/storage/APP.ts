(function () {
  ...
  let inMenu = 1,
    menuDirty = 1;
  let currentModule: MesmetronModule | null = null,
    titleModule: MesmetronTitleModule | null = null;

  function loadModule(filename: string): MesmetronModule {
    return (
      eval(
        require('fs').readFileSync(BASE_PATH + filename),
      ) as () => MesmetronModule
    )();
  }
  ...
  function menuInput(dir: number): void {
    if (dir) {
  ...
  function onKnob1(dir: KnobDirection, long: boolean | undefined): void {
    ...
    if (dir === 0 && long) {
      ...
      inMenu = 1;
      menuDirty = 1;
      titleModule!.repaint && titleModule!.repaint();
  ...
  h.clear();
  titleModule = loadModule('TITLE.JS') as MesmetronTitleModule;
  titleModule!.init(0);
  Pip.audioStart(BASE_PATH + 'WILD.WAV', { repeat: true });
