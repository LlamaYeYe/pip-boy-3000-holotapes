...
(function () {
  ...
  let selectedRow = 0;
  let dirtyDisplay = 1;
  let dirtyCells: Array<{ col: number; row: number }> = []; // list of {col, row} cells needing redraw

  function markDirty(col: number, row: number): void {
    dirtyCells.push({ col: col, row: row });
  }

  // ── Helpers ──────────────────────────────────────────────────────────────────
  function getButtonLabel(col: number, row: number): string {
  ...
  function drawButton(col: number, row: number): void {
