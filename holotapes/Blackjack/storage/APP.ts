(function () {
  ...
  function isBust(hand: string[]): boolean {
  ...
  function resolveDealerTurn(): void {
  ...
  function drawHand(hand: string[], startY: number, hideFirst: boolean): void {
    let startX = 70;
    for (let i = 0; i < hand.length; i++) {
      drawCardAt(
        hand[i],
  ...
  function onLeftWheel(d: KnobDirection): void {
    if (gameState === STATES.BET) {
  ...
  function onRightWheel(d: KnobDirection): void {
