...
(function (goTo: (file: string) => void) {
  const C = {
    ...
    TARGETS: [
      'HOLO/VAULT_TEC_TIMER/TIMER.JS',
      'HOLO/VAULT_TEC_TIMER/STOPWATCH.JS',
    ],
    BOX_Y1: 263,
    BOX_Y2: 303,
  };
  ...
  function drawTitleCard(): void {
    ...
    h.setColor(3)
      .setFontMonofonto36()
      .setFontAlign(0, 0)
      .drawString(title, 240, 40);

    h.setColor(1)
      .setFontMonofonto16()
      .setFontAlign(0, 1)
      .drawString('A product of Vault 919', 375, 80);
  }

  // Computes the fixed x-bounds for the TIMER/STOPWATCH boxes
  function layoutSelector(): void {
  ...
  function drawSelector(): void {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      for (let o = 1; o <= 3; o++)
        h.drawRect(bxX1[i] - o, C.BOX_Y1 - o, bxX2[i] + o, C.BOX_Y2 + o);
      h.setColor(3)
        .setFontMonofonto23()
        .setFontAlign(0, 0)
        .drawString(
