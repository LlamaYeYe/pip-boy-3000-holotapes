...
(function (goTo: (file: string) => void) {
  ...
  let focus = 'stack';
  let ticker!: number, redrawInterval!: number;
  ...
  function rowEnabled(r: number): boolean {
  ...
  function draw(): void {
    ...
    if (laps.length) {
      h.setColor(2)
        ...
        .setFontAlign(0, 0)
        .drawString(
          'LAP ' + (lapView + 1) + ' OF ' + laps.length,
          240,
          C.LAPCOUNT_Y,
        );
      h.setColor(focus === 'stack' && row === 2 ? 3 : 2)
        .setFontMonofonto16()
        .setFontAlign(0, 0)
