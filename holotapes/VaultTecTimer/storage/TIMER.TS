...
(function (goTo: (file: string) => void) {
  ...
  let alertW!: number; // computed once at load - half-width of the ALERT button
  let menuCx!: number, startCx!: number; // computed once at load - centers of the MENU/START pair

  function pad(n: number): string {
    return n < 10 ? '0' + n : '' + n;
  }

  function readSoundDir(): string[] {
  ...
  function drawButton(
    ...
    if (enabled && focused) {
      h.setColor(3);
      for (let o = 1; o <= 3; o++)
        h.drawRect(cx - w - o, y1 - o, cx + w + o, y2 + o);
    }
    h.setColor(enabled ? 3 : 2);
    if (font === 28) h.setFontMonofonto28();
    else h.setFontMonofonto18();
  ...
  function drawSet(): void {
    layoutAlertWidth();
    h.setColor(3)
      .setFontMonofonto36()
      .setFontAlign(0, 0)
      .drawString('TIMER', 240, 47);
    ...
    if (pos === 5) {
      if (alertType === 0) {
        h.setColor(2)
          ...
          .drawString(
            browsingSound
              ? 'Press left wheel to confirm changes'
              : 'Press left wheel to change sound, turn right wheel to use LED',
            240,
            C.ALERT_Y2 + 18,
          );
      } else {
  ...
  function drawRunning(): void {
    ...
    h.setColor(3)
      .setFontMonofonto23()
      .setFontAlign(0, 0)
      .drawString('TIMER RUNNING', 240, 55);
    ...
    h.setColor(2)
      .setFontMonofonto16()
      .setFontAlign(0, 0)
      .drawString(alertLabel(), 240, 200);

    h.setColor(2).drawRect(120, 230, 240, 254);
    if (runRow === 0) {
      h.setColor(3);
      for (let o = 1; o <= 3; o++)
        h.drawRect(120 - o, 230 - o, 240 + o, 254 + o);
    }
    h.setColor(3)
      .setFontMonofonto18()
      .setFontAlign(0, 0)
      .drawString('CANCEL', 180, 242);

    h.setColor(2).drawRect(260, 230, 380, 254);
    if (runRow === 1) {
      h.setColor(3);
      for (let o = 1; o <= 3; o++)
        h.drawRect(260 - o, 230 - o, 380 + o, 254 + o);
    }
    h.setColor(3)
      .setFontMonofonto18()
      .setFontAlign(0, 0)
      .drawString('MENU', 320, 242);
  }
  ...
  function drawPaused(): void {
    ...
    h.setColor(3)
      .setFontMonofonto23()
      .setFontAlign(0, 0)
      .drawString('TIMER PAUSED', 240, 50);
    h.setFontMonofonto36()
      .setFontAlign(0, 0)
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (runRow === i) {
        h.setColor(3);
        for (let o = 1; o <= 3; o++)
          h.drawRect(cx - 48 - o, y1 - o, cx + 48 + o, y2 + o);
      }
      h.setColor(3)
        .setFontMonofonto16()
        .setFontAlign(0, 0)
        .drawString(labels[i], cx, (y1 + y2) / 2);
    }
  }
  ...
  function drawDone(): void {
    h.setColor(3)
      .setFontMonofonto36()
      .setFontAlign(0, 0)
      .drawString("** TIME'S UP! **", 240, 124);
    h.setColor(2)
      .setFontMonofonto28()
      .setFontAlign(0, 0)
      .drawString('PRESS LEFT WHEEL TO STOP', 240, 214);
  }
  ...
  function goToMenuFromRunning(): void {
    stopTicker();
    mode = C.MODES.PAUSED;
    runRow = 0;
    h.clear(1);
    h.setColor(3)
      .setFontMonofonto23()
      .setFontAlign(0, 0)
      .drawString('TIMER PAUSED', 240, 150);
    h.flip();
    Pip.lastFlip = getTime();
  ...
  function adjustFieldOrExit(dir: number): boolean {
  ...
  function horizontalNav(dir: number): boolean {
    ...
    if (pos <= 2) {
      ...
      alertType =
        (alertType + dir + C.ALERT_TYPES.length) % C.ALERT_TYPES.length;
