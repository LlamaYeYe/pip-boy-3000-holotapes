...
(function () {
  ...
  function CommanderDamageSource(
    this: MtgCommanderDamage,
    index: number,
    sourceName: string,
  ): void {
    this.index = index;
  ...
  function drawMenuBoundaries(): void {
  ...
  function drawGameBoundaries(): void {
    switch (playerCount) {
      case 1:
        drawBoundaries(SCREEN_XY);
        break;
  ...
  function drawAppTitleAndVersion(): void {
  ...
  function drawMenuHeader(text: string): void {
  ...
  function drawMenu(menuOptions: MtgMenuOption[]): void {
    ...
    menuOptions.forEach((option: MtgMenuOption, index: number) => {
      ...
      if (
        ...
        (option as MtgCommanderDamage).index === menuIndexSelected
      ) {
        menuOption =
          ' ' +
          (option as MtgCommanderDamage).fromSource +
          ': < ' +
          (option as MtgCommanderDamage).amount +
          ' >';
      ...
      h.drawString(menuOption as string, MENU_XY.x1 + padding, y);
  ...
  function drawMenuHelp(): void {
    ...
    helpTextLines.forEach((line: string | number, index) => {
      const y = MENU_XY.y1 + index * rowHeight + padding;
  ...
  function drawPlayerTile(
    ...
    let strLifeTotal: number | string = player.currentLife;
  ...
  function menuLoad(menuOptions: MtgMenuOption[], menuHeader: string): void {
    menuIndexSelected = 0;
    drawMenuHeader(menuHeader);
    drawMenu(menuOptions);
  }

  function menuScroll(dir: number): void {
  ...
  function playerScroll(dir: number): void {
  ...
  function gameStart(): void {
    ...
    for (let i = 0; i < playerCount; i++) {
      const player = new (Player as MtgPlayerConstructor)(
        i,
        'Player ' + (i + 1),
        startingHP,
      );
      ...
      for (let j = 0; j < (playerCount === 1 ? 6 : playerCount); j++) {
        player.commanderDamageSources.push(
          new (CommanderDamageSource as MtgCommanderDamageConstructor)(
            j,
            'Player ' + (j + 1),
          ),
        );
  ...
  function handleLeftKnob(dir: KnobDirection, long: boolean | undefined): void {
    if (dir !== 0) {
      let now = Date.now();
  ...
  function handleRightKnob(dir: KnobDirection): void {
    let prevIndex = 0;
    if (inGame === false) {
