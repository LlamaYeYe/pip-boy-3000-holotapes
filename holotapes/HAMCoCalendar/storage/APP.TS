(function () {
  ...
  let view = 'CALENDAR';
  let isTyping = false;
  let listScroller: { remove?: () => void } | null = null;
  let activeOverlay: PipDateTimePicker | PipKeyboard | null = null; // Tracks open keyboards/pickers for clean exits
  ...
  function openEditor(): void {
    ...
    activeOverlay = Pip.createKeyboard(
      startText,
      'ADD TASK OR NOTE',
  ...
  function openNotesList(): void {
    ...
    listScroller = Pip.createScroller({
      ...
      onClick: function (index: number) {
        ...
        if (listScroller && listScroller.remove) listScroller.remove();
        listScroller = null;
        Pip.onExclusive('knob1', onKnob1);
        Pip.onExclusive('knob2', onKnob2);
        draw();
  ...
  function openDatePicker(): void {
    ...
    activeOverlay = Pip.createDateTimePicker(
      jumpDate,
      true,
      'JUMP TO DATE',
      function (newDate) {
  ...
  function draw(): void {
    ...
    if (view === 'CALENDAR') {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        try {
          let imgData = fs.readFileSync(
            STORAGE_DIR + '/' + monthFiles[targetMonth],
          );
      ...
      let taskText =
        tasks[currentKey] || 'No logs entry found. Press Knob 1 to add.';

      if (!isTyping) {
        isTyping = true;
        // The typewriter effect is initialized here[cite: 9]
        Pip.typeText(taskText, 20, 75, W - 40, 200, 'Monofonto16').then(
          function () {
            isTyping = false;
          },
        );
  ...
  function onKnob1(dir: KnobDirection, long: boolean | undefined): void {
    ...
    if (view === 'CALENDAR') {
      if (dir === 1) {
        dDay++;
        let maxD = new Date(dYear, dMonth + 1, 0).getDate();
        if (dDay > maxD) {
          dDay = 1;
          dMonth++;
          if (dMonth > 11) {
        ...
        if (dDay < 1) {
          ...
          dDay = new Date(dYear, dMonth + 1, 0).getDate();
          ensureVisible();
        }
        Pip.playSound('SCROLL');
        draw();
