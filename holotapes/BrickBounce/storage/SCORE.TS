(function () {
  ...
  function md(r: BrickBounceScoreRow | 0): 'ARCADE' | 'CLASSIC' {
    return r && r.m ? 'ARCADE' : 'CLASSIC';
  }
  function di(
    r: BrickBounceScoreRow | 0,
  ...
  function hi(): number {
    ns();
    return scores.length ? scores[0].s || 0 : 0;
  }
  function ld(): void {
    let s = ds()(SF);
    scores =
      s && s.scores
        ? (s.scores as BrickBounceScoreRow[])
        : s && s.high
          ? [
              {
                n: '---',
