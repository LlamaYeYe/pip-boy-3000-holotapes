(function () {
  ...
  let kbd: PipKeyboard | undefined,
    kbdKnob1: KnobHandler | undefined,
    kbdKnob2: KnobHandler | undefined;
  let kbdRow = 0,
    kbdCol = 0,
    kbdShift = false;
  ...
  function render(hl: null | number | undefined): void {
  ...
  function sendKey(code: number, mod: number, hl?: number): void {
    if (!code) return;
    if (!kbd) render(hl);
  ...
  function onKbdKnob1(dir: KnobDirection, long: boolean | undefined): void {
    if (!dir) {
      if (long) {
        closeKeyboard();
        return;
    ...
    if (kbdKnob1) kbdKnob1(dir);
  }

  function onKbdKnob2(dir: KnobDirection): void {
    if (!dir) return;
    kbdCol = (kbdCol + dir + 14) % 14;
    if (kbdKnob2) kbdKnob2(dir);
  }
  ...
  function grabKnob(event: string): KnobHandler {
    const store = Pip as PipController &
      Record<string, KnobHandler | KnobHandler[] | undefined>;
    const l = store['#on' + event];
    return (Array.isArray(l) ? l[0] : l) as KnobHandler;
  }
