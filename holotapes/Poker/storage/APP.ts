...
(function () {
  ...
  function getHandSize(m: number): 5 | 7 {
    return m === 1 ? 7 : 5;
  }
  ...
  function dealOneCard(): PokerCard {
  ...
  function rankIdx(c: PokerCard): number {
    return c % 13;
  }
  function suitIdx(c: PokerCard): number {
    return (c / 13) | 0;
  }
  function rankVal(c: PokerCard): number {
    let r = rankIdx(c);
    return r === 0 ? 14 : r + 1;
  }
  ...
  function scoreHand(cards: PokerCard[]): number[] {
    let ranks: number[] = [],
      suits: number[] = [];
    for (let i = 0; i < 5; i++) {
  ...
  function bestScore(hand: PokerCard[]): number[] | null {
  ...
  function dealerDiscards(hand: PokerCard[]): boolean[] {
    let counts: PokerCountMap = {};
  ...
  function applyDraw(hand: PokerCard[], marks: boolean[]): void {
  ...
  function drawHand(hand: PokerCard[], startY: number, hideAll: boolean): void {
    for (let i = 0; i < hand.length; i++) {
  ...
  function drawHeart(cx: number, cy: number, r: number): void {
    h.fillCircle(cx - r * 0.5, cy - r * 0.3, r * 0.5);
    h.fillCircle(cx + r * 0.5, cy - r * 0.3, r * 0.5);
    h.fillPoly([cx - r, cy - r * 0.1, cx + r, cy - r * 0.1, cx, cy + r]);
  }

  function drawSpade(cx: number, cy: number, r: number): void {
    h.fillCircle(cx - r * 0.5, cy + r * 0.3, r * 0.5);
    h.fillCircle(cx + r * 0.5, cy + r * 0.3, r * 0.5);
    h.fillPoly([cx - r, cy + r * 0.1, cx + r, cy + r * 0.1, cx, cy - r]);
    h.fillPoly([
  ...
  function drawClub(cx: number, cy: number, r: number): void {
  ...
  function drawResultScreen(): void {
    h.setColor(3)
      .setFontMonofonto18()
      .setFontAlign(-1, 0)
      .drawString('CHIPS: ' + chips, 30, 29);
