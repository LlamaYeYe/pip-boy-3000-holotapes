...
(function () {
  ...
  function checkWin(): 'X' | 'O' | 'Draw' | null {
    const lines = [
      board[0],
      board[1],
      board[2],
    ...
    for (let line of lines) {
      if (line.every((cell: string) => cell === 'X')) return 'X';
      if (line.every((cell: string) => cell === 'O')) return 'O';
    }
  ...
  function getBestMove(): { x: number; y: number } | null {
    let bestScore = -Infinity;
    let bestMove: { x: number; y: number } | null = null;

    let availableMoves: Array<{ x: number; y: number }> = [];
    for (let y = 0; y < 3; y++) {
      for (let x = 0; x < 3; x++) {
        if (board[y][x] === '') {
          availableMoves.push({
  ...
  function showGameOverMessage(result: string): void {
    if (result === 'Draw') {
      playSound(snd_Draw);
  ...
  function cpuMove(): void {
    ...
    let move!: { x: number; y: number } | { x: number; y: number } | null;
  ...
  let leftWheelPressWatch: number | undefined;

  function onGameKnob1(val: KnobDirection): void {
    if (gameOver) return;
    cursorY = val > 0 ? (cursorY + 1) % 3 : (cursorY + 2) % 3;
    drawBoard();
  }

  function onGameKnob2(val: KnobDirection): void {
    if (gameOver) return;
    cursorX = val > 0 ? (cursorX + 1) % 3 : (cursorX + 2) % 3;
    drawBoard();
  }

  function onMenuKnob1(val: KnobDirection): void {
    if (val > 0) {
