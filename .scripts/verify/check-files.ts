...
const rootDir = path.join(process.cwd(), 'dist', 'pip-boy-3000-holotapes');
...
function referencedPaths(metadata: RawMetadata): string[] {
  ...
  for (const entry of metadata.storage ?? []) {
    paths.push(entry.source, entry.previewMp3, entry.previewMp4);
  }
...
async function main(): Promise<void> {
  ...
  const generatedScripts = (
    await fs.readdir(rootDir, { recursive: true, withFileTypes: true })
  ).filter(
    (entry) =>
      entry.isFile() &&
  ...
  for (const entry of generatedScripts) {
    ...
    const hasFunctionWrapper = trimmed.includes('(function');
    if (
      source.charCodeAt(0) === 0xfeff ||
      (entry.name === 'APP.JS' &&
        (!hasFunctionWrapper || !trimmed.endsWith('});')))
    ) {
      invalidScripts.push(path.relative(rootDir, filePath));
    }
  }
  ...
  for (const entry of invalidScripts) {
    process.stderr.write(
      `Invalid generated script wrapper or UTF-8 BOM: ${entry}\n`,
    );
  }
  ...
  process.stdout.write(
    `Checked ${metadataFiles.length} holotape` +
      ...
      `${invalidScripts.length} invalid script wrapper${invalidScripts.length === 1 ? '' : 's'}.\n`,
  );
